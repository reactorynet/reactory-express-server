  ...
  try {
    if (installedModule && installedModule.services) {
      if (installedModule.services) {
        logger.debug(
          `Module ${installedModule.name}: (${installedModule.services.length}) services installed`,
        );
      ...
      logger.debug(
        `Module ${installedModule || "NULL-MODULE!"} has no services available`,
      );
    }
  } catch (serviceInstallError) {
    logger.error(`Service install error: ${serviceInstallError.message}`, {
      serviceInstallError,
    });
  }
});
        ...
        if (
          ...
          try {
            ...
            context.warn(
              ...
              { service_id: serviceId, props, setterError },
              "warning",
            );
          }
        } else {
  ...
  for (const service of services) {
    ...
    try {
      if (!service || !service.id) {
        logger.warn(
          `Service definition is missing or does not have an id. Skipping service startup.`,
          { service },
        );
        continue;
      }
      logger.debug(`Initializing service ${service.id} (${service.name})`);
      const instance = getService(service?.id as string, props, context);
      logger.debug(`Starting service ${service.id} (${service.name})`, {
        service,
      });

      if (!instance) {
        logger.warn(
          `Service instance for ${service.id} could not be created. Skipping service startup.`,
          { service },
        );
        continue;
      }
      ...
      if (instance.onStartup && typeof instance.onStartup === "function") {
        try {
          ...
          logger.error(
            `An error occured while starting service ${service.id}, please check log for details`,
            serviceStartupError,
          );
          return false;
        }
      ...
      logger.error(
        `An error occured while initializing service ${service.id}, please check log for details`,
        serviceError,
      );
      return false;
    }
