...
function azureUpload(req, callback) {
  let azureProps = {
    account: req.query.account || defaultAzureProps.account,
  ...
  const azureStorage = new AzureBlobStorage(azureProps);
  azureStorage.mkdir(fileRoute); // ensure directory exists
  ...
  busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
    ...
    diskWriterStream.on('finish', () => {
      ...
      azureStorage.writeFile(fileRoute, randomName, fs.readFileSync(saveToPath));
      if (!status) {
