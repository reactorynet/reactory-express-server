...
if [[ -z "$APP_DIR" || ! -f "${APP_DIR}/index.js" ]]; then
  echo "❌ [run-otel] Error: Compiled application not found!" >&2
...
# Resolve OpenTelemetry instrumentation file
OTLP_ENTRY="${APP_DIR}/modules/reactory-telemetry/reactory.inst.otlp.js"
if [[ ! -f "$OTLP_ENTRY" ]]; then
  echo "❌ [run-otel] Error: OpenTelemetry instrumentation entry point not found!" >&2
  echo "   Expected at: ${OTLP_ENTRY}" >&2
  echo "   Ensure 'reactory-telemetry' is installed and compiled." >&2
  exit 1
fi
...
if [[ "$USE_BUN" == "true" ]]; then
  if type ensure_bun &>/dev/null; then
    ...
    echo "❌ [run-otel] Error: Bun is not installed." >&2
    exit 1
  fi
  JS_RUNTIME="bun"
